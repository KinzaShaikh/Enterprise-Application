...
const addNewStudent= (req,res)=>{
    ...
    image.mv(path.join(__dirname,'../public/images',image.name),(error)=>{
        if(!error){
            Student.create({fullname:fullname,age:age,department:department,address:address,image:image.name},(error)=>{
                if(!error){
...
const studentUpdated = (req,res)=>{
    ...
    image.mv(path.resolve(__dirname,"../public/images",image.name),(error)=>{
        Student.findByIdAndUpdate(id,{fullname:fullname,age:age,department:department,address:address,image:image.name},(error,data)=>{
            if(!error){
                res.redirect('displayStudents')
            }else{
                res.redirect('updateStudent')
